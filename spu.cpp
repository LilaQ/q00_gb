...
uint8_t Amplitude = 64;				//	volume
bool SoundIsPlaying = false;

int cycle_count = 0;

std::vector<uint8_t> SC1buf;
std::vector<uint8_t> SC2buf;
...
internal void SDLInitAudio(int32_t SamplesPerSecond, int32_t BufferSize)
{
	...
	AudioSettings.format = AUDIO_U8;		//	One of the modes that doesn't produce a high frequent pitched tone when having silence
	AudioSettings.channels = 2;
	//AudioSettings.callback = func_callback;
...
void stepSPU(unsigned char cycles) {
	...
	if (SC1buf.size() >= 4096) {
		...
		SDL_QueueAudio(1, SC1buf.data(), SC1buf.size());
